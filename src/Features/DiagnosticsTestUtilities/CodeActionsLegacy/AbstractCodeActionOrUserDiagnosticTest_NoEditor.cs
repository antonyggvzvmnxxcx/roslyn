...
public abstract partial class AbstractCodeActionOrUserDiagnosticTest_NoEditor<
    ...
    TProject,
    TSolution,
    TTestWorkspace>(ITestOutputHelper logger = null)
...
#pragma warning restore
    ...
    protected virtual void InitializeWorkspace(TTestWorkspace workspace, TestParameters parameters)
    {
    }
    ...
    protected TTestWorkspace CreateWorkspaceFromOptions(string workspaceMarkupOrCode, TestParameters parameters = null)
    {
    ...
    private protected virtual TTestWorkspace CreateWorkspace(string workspaceMarkupOrCode, TestParameters parameters, TestComposition composition, IDocumentServiceProvider documentServiceProvider)
    {
        var result = TestWorkspace.IsWorkspaceElement(workspaceMarkupOrCode)
    ...
    private static void AddAnalyzerConfigDocumentWithOptions(TTestWorkspace workspace, OptionsCollectionAlias options)
    {
    ...
    internal abstract Task<CodeRefactoring> GetCodeRefactoringAsync(TTestWorkspace workspace, TestParameters parameters);
    ...
    internal async Task<Tuple<Solution, Solution>> TestActionAsync(
        TTestWorkspace workspace, string expected,
        CodeAction action,
    ...
    internal async Task<ImmutableArray<CodeActionOperation>> VerifyActionAndGetOperationsAsync(
        TTestWorkspace workspace, CodeAction action, TestParameters parameters = null)
    {
        parameters ??= TestParameters.Default;
